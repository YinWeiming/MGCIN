...
def main():
    treePath = os.path.join("D:\python项目或代码\AI\\rumorDc\BiGCN_master\data\Weibo\weibotree.txt")
    ...
    labelPath = os.path.join("D:\python项目或代码\AI\\rumorDc\BiGCN_master\data\Weibo\weibo_id_label.txt")
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            save_folder = os.path.join("D:\python项目或代码\AI\\rumorDc\BiGCN_master","data\Weibograph")
            os.makedirs(save_folder, exist_ok=True)
            with torch.no_grad():
             np.savez(os.path.join("D:\python项目或代码\AI\\rumorDc\BiGCN_master",'data/Weibograph/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
            return None
        x_word, x_index, tree, rootfeat, rootindex = constructMat(event)
        x_x = getfeature(x_word, x_index)
