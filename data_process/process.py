import os
from rumorDc.BiGCN_master.Process.dataset import GraphDataset,BiGraphDataset,UdGraphDataset
cwd=os.getcwd()
...
def loadTree(dataname):
    if 'Twitter' in dataname:
        treePath = os.path.join(cwd,'D:\python项目或代码\AI\\rumorDc\BiGCN_master\/data/'+dataname+'/data.TD_RvNN.vol_5000.txt')
...
def loadBiData(dataname, treeDic, fold_x_train, fold_x_test, TDdroprate,BUdroprate):
    data_path = os.path.join('D:\python项目或代码\AI\\rumorDc\BiGCN_master\data', dataname + 'graph')
    print("loading train set", )
